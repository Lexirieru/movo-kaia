...
import { loadAllWithdrawHistory, fetchEscrowsFromIndexer, fetchReceiverDashboardData, testGoldskyConnection } from "../api/api";
...
function DynamicDashboard({ userRole, effectiveWalletAddress, onRoleChange }: DynamicDashboardProps) {
    ...
    const checkUserData = async () => {
      ...
      try {
        ...
        const combinedEscrows = [
          ...senderEscrowsData.map((escrow: any) => ({ ...escrow, type: 'created' })),
          ...receiverDataResult.availableWithdrawals.map((escrow: any) => ({ ...escrow, type: 'claimable' }))
        ];
  ...
  return (
    <div className="space-y-8">
      <div className="text-center mb-8">
        ...
        <p className="text-gray-400">Manage your escrows and claim your payments</p>
      </div>
      ...
      {process.env.NODE_ENV === 'development' && (
        <div className="bg-gray-800/50 border border-gray-600 rounded-xl p-4 text-xs text-gray-400">
          ...
          <div>Claimable Escrows: {receiverData?.availableWithdrawals?.length || 0}</div>
