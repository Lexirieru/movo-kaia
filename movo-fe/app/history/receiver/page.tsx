...
export default function ReceiverHistoryPage() {
  ...
  useEffect(() => {
    ...
    const fetchWithdrawHistory = async () => {
      try {
        const historyTemplate = await loadAllWithdrawHistory(user._id, currentWalletAddress);
        ...
        const templatesWithdrawHistory: WithdrawHistory[] = historyTemplate.map((w: any) => ({
  ...
  const totalSelectedAmount = selectedWithdrawsData.reduce((acc, w) => acc + Number(w.amount), 0);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-black via-gray-900 to-black">
      ...
      <div className="container mx-auto px-4 py-6 space-y-6">
        {/* Action Header */}
        <div>
          <h2 className="text-2xl font-bold text-white mb-1">Your Claims & Withdrawals</h2>
          <p className="text-gray-400 text-sm">Track and manage your payment history</p>
        </div>
        ...
        <SearchFilterBar
          ...
          statusFilter={{
            ...
            onChange: (value: string) => setFilterType(value as "all" | "pending" | "completed")
          }}
          dateFilter={{
            value: dateFilter,
            options: dateFilterOptions,
            onChange: (value: string) => setDateFilter(value as "all" | "week" | "month" | "year")
          }}
            ...
            <div className="lg:hidden space-y-4">
              {filteredWithdraws.map((transaction, index) => (
                <TransactionCard
                  ...
                  isSelected={selectedWithdraws.includes(transaction.withdrawId ?? "")}
